...
abstract class AuthenticationPlugin
{
    ...
    public function getErrorMessage($failure)
    {
        if ($failure == 'empty-denied') {
            if ($GLOBALS['login_user_is_root'] && ($GLOBALS['login_server'] == 'MariaDB 5' || $GLOBALS['login_server'] == 'MariaDB 10') ) {
				return __('You cannot log in without a password. Please sign in to your DSM as administrator, and go to <b>'
                    . $GLOBALS['login_server']
                    . ' </b> > <b>Change Password</b> to set up the password.'
                );
			}
            return __(
    ...
    public function checkRules()
    {
        ...
        if (! $cfg['Server']['AllowNoPassword']
            ...
            $GLOBALS['login_server'] = $cfg['Server']['verbose'];
            $GLOBALS['login_user_is_root'] = ($cfg['Server']['user'] == 'root');
