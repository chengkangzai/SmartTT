        ...
        <div class="card-body">
            <form action="{{ route('settings.update', 'general') }}" method="post" id="storeForm">
                ...
                <div class="mb-3">
                    ...
                    <select name="default_language" id="default_language" class="form-select">
                        @foreach ($setting->supported_language as $language)
                            <option
                                value="{{ $language }}" @selected(old('default_language', $setting->default_language) == $language)>
                                {{ $language }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="default_timezone" id="default_timezone" class="form-select">
                        @foreach ($viewBag['timezones'] as $timezone)
                            <option
                                value="{{ $timezone }}" @selected(old('default_timezone', $setting->default_timezone->getName()) == $timezone)>
                                {{ $timezone }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="row mb-md-2">
                    ...
                    <div class="col-12 mb-2 mb-md-0 col-md-6">
                        <label for="default_currency_symbol">{{ trans('setting.general.default_currency_symbol') }}</label>
                ...
                <div class="mb-3">
                    ...
                    <select name="default_country" id="default_country" class="form-select">
                        @foreach ($viewBag['countries'] as $country)
                            <option
                                value="{{ $country->name }}" @selected(old('default_country', $setting->default_country) == $country->name)>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
