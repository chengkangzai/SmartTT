        ...
        <div class="flex grow flex-col gap-6 md:basis-2/3">
            @forelse ($tours as $tour)
                <a href="{{ route('front.tours', $tour) }}"
                    class="flex w-full flex-col gap-4 rounded-lg border-2 border-gray-200 p-4 no-underline shadow-lg transition duration-300 hover:shadow-xl md:flex-row">
                    <div class="max-w-screen-sm md:max-w-xs">
                        <img srcset="{{ $tour->getFirstMedia('thumbnail')?->responsiveImages()?->getSrcset() ?? '#' }}"
                            src="{{ $tour->getFirstMediaUrl('thumbnail') }}" alt="Image of {{ $tour->name }}"
                            class="aspect-video rounded-t-lg md:rounded-t-none md:rounded-l-lg" />
                    </div>
                    <div class="container mx-auto flex flex-col justify-between">
                        <div
                            class="pb-1 text-2xl font-bold transition duration-300 hover:text-indigo-500 md:py-2 md:text-3xl">
                            {{ $tour->name }}
                        </div>
                        <div>
                            @foreach ($tour->countries as $country)
                                <span class="m-1 inline-block rounded-full bg-amber-200 py-1 px-4 text-sm font-bold">
                                    {{ $country->name }}
                                </span>
                            @endforeach
                        </div>
            ...
            @if ($stillCanLoad)
                <div class="container mx-auto py-4 text-center">
                    <button
                        class="mx-auto inline-flex items-center space-x-2 rounded-full bg-indigo-500 px-5 py-3 text-white transition duration-300 hover:bg-indigo-600 focus:outline-none focus:ring focus:ring-indigo-200"
                        wire:click="loadMore">
